...
> 最后更新: 2026-07-04 | 基于代码探索推导，非既有文档。
...
│   ├── scoring.py          # 评分式 provider 选择引擎（7 个维度）
│   ├── corpus.py           # 素材库语料管理
│   ├── clip_embedder.py    # CLIP 语义嵌入——用于语义搜索
│   ├── delivery_promise.py # 质量关卡：交付承诺校验
│   ├── slideshow_risk.py   # 幻灯片风险评分（6 个维度）
...
│
├── tools/                  # 94 个 Python 工具实现（持续增长中）
│   ├── base_tool.py        # 抽象基类——工具契约
│   ├── tool_registry.py    # 自动发现的单例注册表
│   ├── cost_tracker.py     # 预算治理（估算→预留→结算）
│   ├── analysis/           # 转写、场景检测、帧采样、视频理解
│   ├── audio/              # TTS（ElevenLabs、OpenAI、Piper、MiniMax、Doubao、VoxCPM）、音乐、混音、增强
│   ├── avatar/             # 数字人动画、口型同步
│   ├── capture/            # 屏幕录制、捕获选择器
│   ├── character/          # SVG 骨架、姿势库、动作时间线（卡通动画流水线）
│   ├── enhancement/        # 超分、背景移除、人脸增强/修复、调色、绿幕
│   ├── graphics/           # 图像生成（FLUX、Imagen、Grok、GPT Image、Recraft、MiniMax、ComfyUI、本地扩散模型）、素材库、图表
│   ├── publishers/         # （预留）
│   ├── subtitle/           # SRT/VTT 字幕生成、字幕烧录
│   ├── video/              # 16 个视频生成 provider、合成、拼接、裁剪、重帧
│   └── _comfyui/           # ComfyUI 适配器（共享客户端 + 工作流模板）
│
...
│
├── .agents/skills/         # 第三层：外部技术技能（约 80 个技能：FFmpeg、HyperFrames、GSAP 等）
...
**分析（11 个）：** transcriber（WhisperX）、scene_detect、frame_sampler、video_understand、video_analyzer、visual_qa、composition_validator、face_tracker、audio_energy、audio_probe、transcript_fetcher

**音频（16 个）：**
- *TTS（8 个）：* elevenlabs_tts、google_tts、openai_tts、piper_tts、doubao_tts、voxcpm_tts、minimax_tts、tts_selector
- *音乐（6 个）：* music_gen、suno_music、minimax_music、freesound_music、pixabay_music、music_library
- *处理（2 个）：* audio_mixer、audio_enhance

**数字人（2 个）：** talking_head（SadTalker/MuseTalk）、lip_sync（Wav2Lip）

**角色动画（6 个）：** character_spec_generator、svg_rig_builder、pose_library_builder、action_timeline_compiler、character_rig_renderer、character_animation_reviewer

**屏幕录制（3 个）：** screen_recorder、screen_capture_selector、cap_recorder

**增强（6 个）：** upscale（Real-ESRGAN）、bg_remove（rembg/U2Net）、face_enhance、face_restore（CodeFormer/GFPGAN）、eye_enhance、color_grade（FFmpeg LUTs）

**图像生成（12 个）：** flux_image、grok_image、google_imagen、openai_image、recraft_image、local_diffusion、minimax_image、comfyui_image、pexels_image、pixabay_image、image_selector、image_gen（已弃用）

**图形（3 个）：** code_snippet、diagram_gen、math_animate（ManimCE）

**字幕（2 个）：** subtitle_gen、remotion_caption_burn

**视频生成（19 个）：** grok_video、heygen_video、higgsfield_video、veo_video、kling_video、runway_video、minimax_video、minimax_video_direct、seedance_video、seedance_replicate、wan_video、hunyuan_video、cogvideo_video、ltx_video_local、ltx_video_modal、pexels_video、pixabay_video、comfyui_video、video_selector

**视频后期（8 个）：** video_compose（FFmpeg）、hyperframes_compose、video_stitch、video_trimmer、auto_reframe、green_screen_composite、green_screen_processor、silence_cutter

**发布（1 个）：** export_bundle

**来源与语料（4 个）：** video_downloader、clip_search、direct_clip_search、corpus_builder
...
| `documentary-montage` | hybrid | 从免版权素材库和开放档案库构建主题蒙太奇 |
...
```
第三层: .agents/skills/          外部技术知识（约 80 个技能）
...
| `OPENAI_API_KEY` | openai_tts、openai_image | TTS 回退、GPT Image 2 |
| `XAI_API_KEY` | grok_image、grok_video | Grok 图像编辑/生成、Grok 视频生成 |
| `FAL_KEY` | flux_image、kling_video、veo_video、minimax_video、recraft_image、seedance_video | fal.ai 托管模型（FLUX、Veo、Kling、MiniMax、Recraft、Seedance） |
| `MINIMAX_API_KEY` | minimax_tts、minimax_music、minimax_image、minimax_video_direct | MiniMax 直连 API（TTS、音乐、图像、视频） |
| `HEYGEN_API_KEY` | heygen_video | 多 provider 视频生成网关 |
| `PEXELS_API_KEY` | pexels_image、pexels_video | 素材库媒体资源 |
| `PIXABAY_API_KEY` | pixabay_image、pixabay_video | 素材库媒体资源 |
| `UNSPLASH_ACCESS_KEY` | pexels_image | Unsplash 免版权图片（通过共享素材接口） |
| `GOOGLE_API_KEY` | google_imagen、google_tts | Google Imagen 图像、Google Cloud TTS |
| `GOOGLE_APPLICATION_CREDENTIALS` | google_tts、google_imagen | GCP 服务账号 JSON 密钥文件路径（API Key 的替代方案） |
| `GOOGLE_CLOUD_PROJECT` | google_imagen | GCP 项目 ID（Imagen 通过 Vertex AI 时需要） |
| `GOOGLE_CLOUD_LOCATION` | google_imagen | Vertex AI 区域（默认：us-central1） |
| `DOUBAO_SPEECH_API_KEY` | doubao_tts | 火山引擎 Doubao 语音 TTS |
| `DOUBAO_SPEECH_VOICE_TYPE` | doubao_tts | Doubao 音色/语音类型选择 |
| `SUNO_API_KEY` | suno_music | Suno AI 音乐生成（完整歌曲、任意风格） |
| `RUNWAY_API_KEY` | runway_video | Runway Gen-4 直连 API |
| `HIGGSFIELD_API_KEY` + `HIGGSFIELD_API_SECRET` | higgsfield_video | Higgsfield 多模型视频 |
| `MODAL_LTX2_ENDPOINT_URL` | ltx_video_modal | 自托管 LTX-2 端点 |
| `VIDEO_GEN_LOCAL_ENABLED` | 本地视频工具 | 启用本地 GPU 生成 |
| `VIDEO_GEN_LOCAL_MODEL` | wan、hunyuan、ltx、cogvideo | 选择本地模型（如 wan2.1-1.3b、hunyuan-1.5） |
| `HF_TOKEN` | transcriber | HuggingFace 令牌（启用说话人分离） |
| `WAV2LIP_PATH` | lip_sync | 克隆的 Wav2Lip 仓库路径 |
| `SADTALKER_PATH` | talking_head | 克隆的 SadTalker 仓库路径 |
| `VOXCPM_MODEL` | voxcpm_tts | 可选 VoxCPM 模型覆盖（默认：openbmb/VoxCPM2） |
